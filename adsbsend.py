...
def main():
    ...
    ip_input = input(f"Enter ATC system IP address [{default_ip}]: ").strip()
    atc_ip = ip_input if ip_input else default_ip
    
    # Get ATC system port
    default_port = "30001"
    port_input = input(f"Enter ATC system port [{default_port}]: ").strip()
    try:
    ...
    print()
    print(f"Generating {num_aircraft} {aircraft_type} aircraft...")
    print(f"Target: {atc_ip}:{atc_port}")
    print("-" * 70)
    print()
    ...
    try:
        ...
        print(f"✓ Connected to ATC system at {atc_ip}:{atc_port}")
        print()
    except Exception as e:
        print(f"✗ Could not connect to ATC system: {e}")
        print(f"Make sure ATC system is running at {atc_ip}:{atc_port}")
        return
    ...
    for i in range(num_aircraft):
        ...
        if is_military:
            ...
            callsign = f"{base}{random.randint(1, 99):02d}"
        else:
            airlines = ["AAL", "DAL", "UAL", "SWA", "JBU", "ASA", "FFT", "SKW", 
                       "NKS", "BAW", "AFR", "DLH", "ACA", "QFA", "EIN", "KLM"]
            airline = random.choice(airlines)
            callsign = f"{airline}{random.randint(1000, 9999)}"
        ...
        print(f"  [{i+1:2d}] {callsign:8s} ({icao}) - {type_str} - "
              f"{aircraft_lat:7.3f}, {aircraft_lon:8.3f}, {altitude:5d}ft, "
              f"{speed:3d}kts, {heading:3.0f}°")
        ...
        try:
            ...
            print(f"  ✗ Error sending aircraft {i+1}: {e}")
    
    print()
    print("="*70)
    print(f" {num_aircraft} Aircraft Sent - Starting Continuous Simulation")
    print("="*70)
    print()
    ...
    try:
        ...
        print(f"{'Time':<8} {'Updates':<10} {'Aircraft':<12} {'Position':<28} {'Alt':<8} {'Course':<15}")
        print("-" * 95)
        ...
        while True:
            ...
            if update_count % 5 == 0:
                ...
                print(f"{minutes:02d}:{seconds:02d}    "
                      f"{update_count:<10} "
                      f"{ac['callsign']:<12} "
                      f"{aircraft_lat:7.3f}, {aircraft_lon:8.3f}   "
                      f"{ac['altitude']:<8} "
                      f"{ac['speed']:3d}kts → {heading:3.0f}°")
        ...
        print(f"Total messages: ~{update_count * num_aircraft * 2}")
        print()
    except Exception as e:
        print(f"\nError: {e}")
        import traceback
