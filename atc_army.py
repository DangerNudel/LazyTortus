...
class ADSBReceiver:
    ...
    def _decode_position(self, msg_hex, ac):
        """Decode position from ADS-B message (simplified CPR)"""
        try:
            # Extract altitude
            alt_bits = int(msg_hex[10:13], 16)
            q_bit = (alt_bits >> 4) & 1
            
            if q_bit:
                alt_bits = ((alt_bits & 0x0FE0) >> 1) | (alt_bits & 0x000F)
                altitude = alt_bits * 25 - 1000
            else:
                altitude = 0
            
            # Extract CPR encoded position
            lat_cpr = int(msg_hex[14:19], 16) >> 4
            lon_cpr = ((int(msg_hex[18:23], 16) >> 4) & 0x1FFFF)
            
            # Time bit (even=0, odd=1)
            time_bit = int(msg_hex[13], 16) & 0x04
            
            # Simplified CPR decoding - for educational purposes
            # In real implementation, this would use proper CPR algorithm
            # For now, we'll use a simplified approach that works for local aircraft
            
            # Store frame
            if time_bit:
                ac['position_odd'] = (lat_cpr, lon_cpr, time.time())
            else:
                ac['position_even'] = (lat_cpr, lon_cpr, time.time())
            
            # If we have both frames, decode position
            if ac['position_even'] and ac['position_odd']:
                lat_even, lon_even, time_even = ac['position_even']
                lat_odd, lon_odd, time_odd = ac['position_odd']
                
                # Check if frames are recent (within 10 seconds)
                if abs(time_even - time_odd) < 10:
                    # Simplified CPR decode (works for local airspace)
                    # This is a simplified version for student data
                    dlat_even = 360.0 / 60
                    dlat_odd = 360.0 / 59
                    
                    # Calculate latitude
                    j = int((59 * lat_even - 60 * lat_odd) / 131072 + 0.5)
                    lat_even_decoded = dlat_even * ((j % 60) + lat_even / 131072)
                    lat_odd_decoded = dlat_odd * ((j % 59) + lat_odd / 131072)
            ...
            # If we already have a position, keep it and just update altitude
            if ac['lat'] is not None:
                return ac['lat'], ac['lon'], altitude
    ...
    def _decode_velocity(self, msg_hex):
        ...
        try:
            ...
            ew_sign = (int(msg_hex[11], 16) >> 2) & 1
            ew_vel = ((int(msg_hex[11:13], 16) & 0x3FF))
            
            ns_sign = (int(msg_hex[13], 16) >> 3) & 1
            ns_vel = ((int(msg_hex[13:15], 16) & 0x3FF))
